...
app.get("/", function(req, res){
	res.sendFile(__dirname + "/index.html"); //links to html file CHANGE /.html to you actually html file
...
io.sockets.on("connection", function(socket){
	...
	socket.on("disconnect", function(data){
		...
		memers.splice(memers.Of(socket.username), 10000); //accessing the array memers
		
		
	memeConnections.splice(memeConnections.Of(socket),1);
